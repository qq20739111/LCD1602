...
class LCD1602:
    def __init__(self, name='lcd1620'):
        self.version = "1.0.0"
        self.name = name
        self.__default_pins__ = ["VSS", "VDD", "V0", "RS", "RW", "E", "D0", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "BLA", "BLK"]
        ...
        self.v0_pwm = {"enable": True, "pin_name": self.__default_pins__[2], "freq": 1000, "duty_u16": 32768, "contrast_percent": 50}
        self.bla_pwm = {"enable": False, "pin_name": self.__default_pins__[14], "freq": 1000, "duty_u16": 32768, "brightness_percent": 50}
        self.settings = {
            "cursor_position": 0x00, "ac_auto_increase": True, "display_follow_cursor": False,
            "display_on": True, "cursor_visible": True, "cursor_blink": True,
            "data_trans_bits": 4, "display_lines": 2, "dot_matrix": 7
        }
        self.command = {
            "LCD_CLEARDISPLAY": 0x01, "LCD_RETURNHOME": 0x02,
            "LCD_ENTRYMODESET_1": 0x04, "LCD_ENTRYMODESET_2": 0x05,
            "LCD_ENTRYMODESET_3": 0x06, "LCD_ENTRYMODESET_4": 0x07,
            "LCD_DISPLAYCONTROL_1": 0x08, "LCD_DISPLAYCONTROL_2": 0x09,
            "LCD_DISPLAYCONTROL_3": 0x0A, "LCD_DISPLAYCONTROL_4": 0x0B,
            "LCD_DISPLAYCONTROL_5": 0x0C, "LCD_DISPLAYCONTROL_6": 0x0D,
            "LCD_DISPLAYCONTROL_7": 0x0E, "LCD_DISPLAYCONTROL_8": 0x0F,
            "LCD_CURSORSHIFT_1": 0x10, "LCD_CURSORSHIFT_2": 0x14,
            "LCD_CURSORSHIFT_3": 0x18, "LCD_CURSORSHIFT_4": 0x1C,
            "LCD_FUNCTIONSET_4BIT_1LINE_5x7": 0x20, "LCD_FUNCTIONSET_4BIT_1LINE_5x10": 0x24,
            "LCD_FUNCTIONSET_4BIT_2LINE_5x7": 0x28, "LCD_FUNCTIONSET_4BIT_2LINE_5x10": 0x2C,
            "LCD_FUNCTIONSET_8BIT_1LINE_5x7": 0x30, "LCD_FUNCTIONSET_8BIT_1LINE_5x10": 0x34,
            "LCD_FUNCTIONSET_8BIT_2LINE_5x7": 0x38, "LCD_FUNCTIONSET_8BIT_2LINE_5x10": 0x3C,
            "LCD_SETCGRAMADDR": 0x40, "LCD_SETDDRAMADDR": 0x80,
        }
        self.browser = {
            "content": "", "content_length": 0, "content_max_length": 65536,
            "line_width": 16, "line_count": 0, "line_pointer": 0, "print_speed": 3
        }
        self.is_pin_ready = False
        self.is_write_ready = False
        self.is_read_ready = False
        self.init_by_default()
    def __str__(self): return f"LCD1602(name='{self.name}')"
    def __repr__(self): return "LCD1602()"
    def enable_pin(self, pin_name, mcu_pin_name):
        ...
        if not isinstance(mcu_pin_name, (str, int)):
            raise ValueError(f"Invalid connected pin: {mcu_pin_name}.")
        if pin_name in self.__default_data_pins__ and not isinstance(mcu_pin_name, int) and not mcu_pin_name.isdigit():
            raise ValueError(f"Invalid connected pin: {mcu_pin_name}.")
        if isinstance(mcu_pin_name, str) and mcu_pin_name.isdigit():
            mcu_pin_name = int(mcu_pin_name)
        if isinstance(mcu_pin_name, int) and mcu_pin_name not in self.get_mcu_gpio_pins_list():
            raise ValueError(f"Invalid GPIO pin number: {mcu_pin_name}.")
    ...
    def enable_function_pin(self, pin_name, mcu_pin_name):
        if pin_name not in self.__default_function_pins__:
            raise ValueError(f"Invalid function pin name: {pin_name}.")
    ...
    def enable_data_pin(self, pin_name, mcu_pin_name):
        if pin_name not in self.__default_data_pins__:
            raise ValueError(f"Invalid data pin name: {pin_name}.")
        return self.enable_pin(pin_name, mcu_pin_name)
    ...
    def enable_data_pins_by_default(self):
        ...
        if self.settings["data_trans_bits"] == 4:
            ...
            raise ValueError("Invalid data transmit mode.")
        return True
    ...
    def bind_mcu_pin(self, pin_name):
        if pin_name not in self.enabled_pins:
            raise ValueError(f"Pin {pin_name} is not enabled.")
    ...
    def get_enabled_function_pins_list(self): return [pin for pin in self.__default_function_pins__ if pin in self.enabled_pins]
    ...
    def set_clear(self):
        ...
        time.sleep_ms(2)
        return True
    def clear(self): return self.set_clear()
    ...
    def set_ac_auto_increase(self, mode=True):
        if mode not in [True, False]: return False
    ...
    def set_display_cursor_blink_mode(self):
        ...
        cmds = [
            self.command["LCD_DISPLAYCONTROL_1"], self.command["LCD_DISPLAYCONTROL_2"],
            self.command["LCD_DISPLAYCONTROL_3"], self.command["LCD_DISPLAYCONTROL_4"],
            self.command["LCD_DISPLAYCONTROL_5"], self.command["LCD_DISPLAYCONTROL_6"],
            self.command["LCD_DISPLAYCONTROL_7"], self.command["LCD_DISPLAYCONTROL_8"],
        ]
    ...
    def set_data_trans_bits(self, bits=4):
        if bits not in [4, 8]: raise ValueError("Invalid data transmission mode.")
    ...
    def set_display_lines(self, lines=2):
        if lines not in [1, 2]: return False
        self.settings["display_lines"] = lines
        self.set_data_lines_matrix_mode()
        return True
    def set_dot_matrix(self, size=7):
        if size not in [7, 10]: return False
    ...
    def send_byte(self, value):
        if not isinstance(value, int): raise ValueError("Invalid value type.")
        if self.settings["data_trans_bits"] == 4:
            ...
            raise ValueError("Invalid data transmission mode.")
        return True
    ...
    def print_line(self, text, line=0):
        ...
        if not self.is_write_ready:
            raise ValueError("Write is not ready.")
    ...
    def scroll_line(self, text, line=0, speed=3):
        ...
        self.clear_line(line)
        return True
    def display_on(self): return self.set_display_on(True)
    def display_off(self): return self.set_display_on(False)
    ...
    def show_blink_cursor(self):
        ...
        return True
    def cursor_home(self): return self.set_cursor_return_home()
    ...
    def browser_set_content_max_length(self, content_max_length=1024):
        ...
        return True
    def browser_set_buffer_size(self, content_max_length=1024): return self.browser_set_content_max_length(content_max_length)
    def browser_get_content_max_length(self): return self.browser["content_max_length"]
    def browser_get_buffer_size(self): return self.browser_get_content_max_length()
    def browser_get_content(self): return self.browser["content"]
    def browser_get_content_length(self): return self.browser["content_length"]
    def browser_set_line_width(self, line_width=16):
        if line_width < 1 or line_width > 40: return False
        ...
        return True
    def browser_get_line_width(self): return self.browser["line_width"]
    def browser_get_line_count(self): return self.browser["line_count"]
    def browser_set_line_pointer(self, line_pointer=0):
        if line_pointer < 0: line_pointer = 0
        elif line_pointer >= self.browser["line_count"]: line_pointer = self.browser["line_count"] - 1
        self.browser["line_pointer"] = line_pointer
        return True
    def browser_get_line_pointer(self): return self.browser["line_pointer"]
    ...
    def browser_scroll_1lines(self, line_pointer=None, count=1, line=0, speed=None):
        ...
        if line_pointer is None: line_pointer = self.browser["line_pointer"]
        if line_pointer < 0: line_pointer = 0
        if line_pointer >= self.browser["line_count"]: line_pointer = self.browser["line_count"] - 1
        start_line = line_pointer
        end_line = start_line + count
        if end_line > self.browser["line_count"]: end_line = self.browser["line_count"]
    ...
    def browser_write(self, text):
        ...
        return True
    def browser_print(self, text): return self.browser_write(text)
    ...
    def display_contrast(self, percent):
        if not self.v0_pwm["enable"]:
            raise ValueError("V0 PWM control is not enabled.")
        pin_name = self.v0_pwm["pin_name"]
        if pin_name not in self.enabled_pins:
            raise ValueError(f"Pin {pin_name} is not enabled.")
        if pin_name not in self.bind_mcu_pins:
            raise ValueError(f"Pin {pin_name} is not initialized.")
    ...
    def backlight_brightness(self, percent):
        if not self.bla_pwm["enable"]:
            raise ValueError("BLA PWM control is not enabled.")
